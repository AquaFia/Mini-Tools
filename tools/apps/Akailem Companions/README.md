...
A character may optionally have one `special_module.js` beside `character.js`.
...
The companion automatically attempts to load that file. If it is absent, no special-module tab appears. If it exists and defines `window.CompanionSpecialModule` with an `init(runtime)` function, the shared companion provides the navigation/content mounts and special-mode switching.

A single `special_module.js` may contain multiple internal tools or views. Character colors are inherited through the companion CSS variables; module code should use those variables instead of hardcoding a second palette.

See either template character's `special_module.js` for the current module contract.
