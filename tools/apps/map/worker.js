  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/api/letters") {
        return await listLetters(url, env);
      }
      ...
      const letterMatch = url.pathname.match(/^\/api\/letters\/([0-9a-fA-F-]{32,36})$/);
      if (letterMatch) {
        return await readLetter(letterMatch[1], url, env);
      }
...
async function listLetters(url, env) {
  if (!env.LETTERS_DATABASE_ID) return json({ error: "LETTERS_DATABASE_ID is not configured" }, 500, env);
  const month = Number(url.searchParams.get("month"));
  const day = Number(url.searchParams.get("day"));
  if (!validMonthDay(month, day)) return json({ error: "Valid month and day are required" }, 400, env);

  const rows = await queryAll(env.LETTERS_DATABASE_ID, env);
  const letters = rows.map(page => normalizeLetter(page)).filter(Boolean)
    .filter(letter => isAvailableOn(letter.deliveryDate, letter.expirationDate, month, day))
    .sort((a, b) => monthDayValue(a.deliveryDate) - monthDayValue(b.deliveryDate) || a.subject.localeCompare(b.subject));
  return json({ letters }, 200, env);
}

async function readLetter(pageId, url, env) {
  const month = Number(url.searchParams.get("month"));
  const day = Number(url.searchParams.get("day"));
  if (!validMonthDay(month, day)) return json({ error: "Valid month and day are required" }, 400, env);
  const normalizedId = pageId.replaceAll("-", "");
  const rows = await queryAll(env.LETTERS_DATABASE_ID, env);
  const page = rows.find(item => item.id.replaceAll("-", "") === normalizedId);
  if (!page) return json({ error: "Letter not found" }, 404, env);
  const letter = normalizeLetter(page);
  if (!letter || !isAvailableOn(letter.deliveryDate, letter.expirationDate, month, day)) return json({ error: "Letter is not available today" }, 403, env);
  const state = { count: 0, truncated: false };
  const blocks = await getBlockTree(page.id, env, state, 0);
  return json({ letter, blocks, truncated: state.truncated }, 200, env);
}

function normalizeLetter(page) {
  const p = page.properties || {};
  const deliveryDate = getDate(p["Delivery Date"]);
  const character = getSelect(p["Character"]);
  if (!deliveryDate || !character) return null;
  return {
    id: page.id, subject: getTitle(p["Subject"]) || "Untitled letter", character,
    sender: getPlainText(p["Sender"]), recipient: getPlainText(p["Recipient"]),
    deliveryDate, expirationDate: getDate(p["Expiration Date"]), url: page.url
  };
}
function validMonthDay(month, day) { return Number.isInteger(month) && month >= 1 && month <= 12 && Number.isInteger(day) && day >= 1 && day <= 31; }
function monthDayValue(date) { if (!date) return 0; const m = /^(?:\d{4})-(\d{2})-(\d{2})/.exec(date); return m ? Number(m[1]) * 100 + Number(m[2]) : 0; }
function isAvailableOn(deliveryDate, expirationDate, month, day) {
  const now = month * 100 + day, start = monthDayValue(deliveryDate), end = monthDayValue(expirationDate);
...
function getPlainText(prop) {
  if (!prop) return "";
  const values = prop[prop.type];
  return Array.isArray(values) ? values.map(item => item.plain_text || "").join("").trim() : "";
}
